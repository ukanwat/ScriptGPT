...
if (!openAIKey) {
  throw new Error("Failed to load OPENAI_API_KEY from .env file");
}
var loadEnv_default = openAIKey;
...
// src/utils/buildPromptString.ts
var buildPromptString = (description, spec) => {
  return `Description:
${description}
Specification:
${JSON.stringify(spec)}
...
var buildPromptString_default = buildPromptString;
...
var apiClient = import_axios.default.create({
  ...
  headers: {
    ...
    "Authorization": `Bearer ${loadEnv_default}`
    // Use the imported API key.
  }
});
var apiClient_default = apiClient;

// src/utils/parseCodeFromResponse.ts
var parseCodeFromResponse = (responseContent, language) => {
  const codeStartMarker = "```" + language.toLowerCase();
  const codeEndMarker = "```";
...
// src/services/gptService.ts
var generateCodeWithGPT4 = async (prompt, functionName, isTypescript) => {
...
use upticks '\`\`\`${language.toLowerCase()}' and '\`\`\`' for start and end code markers. The name of the function you should be '${functionName}'` },
        {
          role: "user",
          content: buildPromptString_default(`It takes an array of numbers and returns their sum.`, JSON.stringify(
...
\`\`\`` },
    ...
    const response = await apiClient_default.post("/chat/completions", data);
...
var gptService_default = generateCodeWithGPT4;

// src/services/fileSaver.ts
var import_fs_extra = __toESM(require("fs-extra"));
var import_chalk = __toESM(require_source());
var saveFunctionToFile = async (functionName, functionCode, isTypescript) => {
  var filePath = generatePath + `/${functionName}.${isTypescript ? "ts" : "js"}`;
  const fullCode = `//Auto-generated function
${functionCode};
`;
  try {
    await import_fs_extra.default.writeFile(filePath, fullCode, { encoding: "utf8" });
    console.log(`Function saved successfully to: `, import_chalk.default.green(filePath));
  } catch (error) {
    console.error("Error saving function to file:", import_chalk.default.red(filePath));
    throw error;
  }
};

// src/generateFunction.ts
var import_chalk2 = __toESM(require_source());
var generateFunction = async (functionName, description, spec, isTypescript) => {
  const prompt = buildPromptString_default(description, spec);
  try {
    const generatedCode = await gptService_default(prompt, functionName, isTypescript);
    console.log(`\`${functionName}\` code:
`, import_chalk2.default.blue(generatedCode));
...
var generatePath = import_app_root_path.default + "/generated";
async function generator(generateAll) {
  var isTypescript = import_fs_extra2.default.existsSync(import_app_root_path.default + "/tsconfig.json");
  ;
  var generatedFunctions = [];
  const explorerSync = (0, import_cosmiconfig.cosmiconfigSync)("scriptgpt", {
  ...
  generatePath = config.outputPath ? import_path.default.join(import_app_root_path.default.path, config.outputPath) : generatePath;
  if (!import_fs_extra2.default.existsSync(generatePath)) {
    import_fs_extra2.default.mkdirSync(generatePath, { recursive: true });
  ...
  try {
    ...
    for (const value of jsonData.functions) {
      ...
      var filePath = import_path.default.join(generatePath, "/" + value.name + (isTypescript ? ".ts" : ".js"));
      console.log("outputPath:", filePath);
      console.log("functionName:", value.name);
      if (import_fs_extra2.default.existsSync(filePath) && generateAll === false) {
...
`;
  ...
  fullCode = fullCode + `};`;
  try {
    await import_fs_extra2.default.writeFile(functionsFilePath, fullCode, { encoding: "utf8" });
  } catch (error) {
...
program.command("generate").description("Generate functions").option("-a, --all", "Generate all functions").action(async (options) => {
