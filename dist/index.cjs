...
function extractLibraries(input, action) {
  const regex = new RegExp(`${action}:([^\\s]+)`, "g");
  const matches = input.match(regex);
  if (matches) {
    const libraries = matches.map((match) => match.split(":")[1]);
    return libraries;
  } else {
    return [];
  }
...
var installLibrary = (library) => {
  (0, import_child_process.exec)(`npm install ${library}`, (error, stdout, stderr) => {
    if (error) {
...
\`\`\`` },
    ...
    if (response.data.choices && response.data.choices.length > 0) {
      ...
      if (choice && choice.message && choice.message.content) {
        const libsToInstall = extractLibraries(choice.message.content, "INSTALL");
